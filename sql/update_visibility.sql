  ...
  set l_visible=exists (
    select * from locations sl
      ...
      where
        ...
        and locations.l_cid!=sl.l_cid and locations.l_bid=sl.l_bid and sl.l_gid=locations.l_gid and m_capture
  ), l_blocking=exists (
    select * from locations sl
      join moves on m_pid=sl.l_pid
      join entrants on e_gid=l_gid and e_bid=locations.l_bid
      where
        ...
        and locations.l_cid!=sl.l_cid and locations.l_bid=sl.l_bid and sl.l_gid=locations.l_gid and m_move
  )
  where l_gid=:gid
  and exists (
